...
export const getSchedStudent = async (group, parsedDate1 , parsedDate2 ) => {
    ...
    let dateOfParas = (number) => {
        ...
        for(let i = 2; null !== cheer(`#timeTableGroup > tbody > tr:nth-child(${number}) > td:nth-child(2) > div:nth-child(${i})`).html(); i++ ) {
            let time = `${cheer(`#timeTableGroup > tbody > tr:nth-child(${number}) > td:first-child > div:nth-child(${i})> .start`).html()} - ${cheer(`#timeTableGroup > tbody > tr:nth-child(${number}) > td:first-child > div:nth-child(${i})> .finish`).html()}`
            let info = {
